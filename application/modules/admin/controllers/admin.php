...
class Admin extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('tipe') == 'tbl_admin') {
            $data['header'] = 'header';
    ...
    function simpanidadmin() {
        ...
        $query = $this->db->query("SELECT * FROM tbl_admin WHERE username='$usernamelama' AND userpass='$passwordlama'");
    ...
    function tabelabsensi() {
        ...
        for ($kelas = 1; $kelas <= 18; $kelas++) {
            ...
            foreach ($qsakit as $row) {
                $sakit[$kelas] = $row[0];
            }
            foreach ($qizin as $row) {
                $izin[$kelas] = $row[0];
            }
            foreach ($qalpha as $row) {
                $alpha[$kelas] = $row[0];
            }
        }
